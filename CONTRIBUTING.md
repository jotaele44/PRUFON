...
# Contributing to ovnis-pr

Thanks for helping improve `ovnis-pr` — the historical case-corpus producer for Puerto Rico-centered anomalous-event research in the Puerto Rico
...
| Template drift | Run `python3 tools/render_federation_templates.py --repo ovnis-pr --repo-root /path/to/ovnis-pr --check` from a local Hub checkout. |
