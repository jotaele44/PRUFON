...
|---|---|---|---|
| D1 | Functional completeness | **3** | 11 backend routes serving a real case corpus; a single but data-rich dashboard page |
| D2 | Data reality | **4** | 1.1 MB of case data, **zero synthetic-flagged files**; ships a populated 1.5 MB offline snapshot |
| D3 | UI craft | **2** | 4.8k LOC but **1 page**; 32 `aria-*` and 8 `role=` (good markup) against 0 empty states and 0 `ErrorBoundary` |
| D4 | Test coverage | **2** | `72 passed` (6.5s), 8 test files — the smallest suite in the federation, though also the smallest codebase |
| D5 | Engineering hygiene | **1** | **No `pyproject.toml`, no ruff, no mypy, and no lint step in any of its 7 workflows.** The weakest tooling baseline of the seven. |
| D6 | Doc accuracy | **3** | 10 markdown files; `federation.json` is accurate; no drift found |

**Overall: good data and honest markup on almost no engineering scaffolding.** The case
corpus is real and the UI markup is more accessible than most siblings. What is missing is
the machinery that keeps a repo healthy as it grows: no package config, no linter, no type
checker, no lint gate.
...
| `server/backend/main.py` (334 LOC) | 11 routes — `/cases`, `/cases/{id}`, `/candidates`, `/geojson`, `/stats`, `/health` |
...
| `scripts/` (11 files) | proportionate; no test module targets them directly |

**SCAFFOLD**

| Item | Why |
|---|---|
| Error and empty UX | 0 `ErrorBoundary` and 0 empty-state components; a failed fetch has no designed surface |
...
|---|---|---|---|
| `Dashboard.jsx` (4,779 LOC across the app) | `lib/api.js` → `/cases`, `/candidates`, `/geojson`, `/stats` | loading (3 files); **no empty state, no error boundary** | **Functional but single-route** |
...
|---|---|---|---|
| 1 | Add `pyproject.toml` with ruff + mypy config, and a lint step in CI | **S** | The single cheapest maturity gain available in the federation. `ruff check .` currently reports 3 findings under default rules — this repo could be clean today and stay clean. `aguayluz-pr`'s config (`E,F,I,B,UP,SIM,W`) is a good template at a comparable size. |
| 2 | Split the dashboard into routed pages | **M** | 4,779 LOC behind one route. `aguayluz-pr` (11 pages) is the closest in-house model. |
| 3 | Add `ErrorBoundary` and empty-state components | **S** | 0 of each today. `skywatcher-pr/frontend/src/components/skywatcher/EmptyState.jsx` is a ready pattern. |
