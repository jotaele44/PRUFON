    ...
    if (root / ".git").exists():
        ...
        if enforce_change_scope:
            ...
            if scope_base_obj.returncode != 0:
                ...
                run_git(root, "merge-base", "--is-ancestor", scope_base, "HEAD").returncode
                != 0
            ):
...
def main() -> int:
    ...
    p.add_argument(
        "--change-base",
        help="commit used for changed-path enforcement (defaults to pinned_base_commit)",
    )
    args = p.parse_args()
    result = validate(
