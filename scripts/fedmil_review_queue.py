...
def build_queue(
    ...
    for candidate in candidates:
        ...
        needs_second = bool(
            assessment
            and assessment.get("review_status") == "needs_second_review"
        )
        queue.append(
            {
                "candidate": candidate,
                "assessment": assessment,
                "state": "second_review" if needs_second else "pending",
            }
        )
    return sorted(
        queue,
        key=lambda row: float(row["candidate"].get("candidate_score", 0)),
        reverse=True,
    )
...
def adjudicate(
    ...
    *,
    ...
    needs_second = (
        classification in {"DIRECT_MATCH", "CONTRADICTORY"}
    ...
    return {
        ...
        + str(candidate["candidate_id"]).split("_", 1)[1],
        ...
        "supports_conventional_explanation": classification
        in {"DIRECT_MATCH", "PLAUSIBLE_CONTEXT"},
        ...
        "reviewed_at": now,
        "review_status": (
            "needs_second_review" if needs_second else "adjudicated"
        ),
        "lineage": {
...
def append_assessment(path: Path, assessment: dict[str, Any]) -> None:
    ...
    rows = [
        row
        for row in rows
        if row.get("candidate_id") != assessment["candidate_id"]
    ]
...
def main() -> int:
    ...
    queue = build_queue(
        ...
        load_jsonl(Path(args.assessments)),
    )
