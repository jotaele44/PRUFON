# Run OVNIS as a desktop app

Double-click the launcher for your system in the repo root:

| System | File |
|---|---|
| macOS | `PRII-OVNIS.command` |
| Windows | `PRII-OVNIS.bat` |
| Linux | `PRII-OVNIS.sh` |
...
`PRII-OVNIS.app` is a double-click macOS app (Apple-silicon and Intel). Double-click
...
- **Terminal (always works).** Paste this into Terminal (pasting a command is
  ...
  ```bash
  xattr -dr com.apple.quarantine "/path/to/ovnis-pr/PRII-OVNIS.app"
  ```
