...
### §0 verification checklist (re-run 2026-05-31 against the 84-row state)
- **Candidate table — 84 rows. Every row has a non-empty `source_url`.** Verified programmatically 2026-05-31: 0 rows missing a source identifier.
  - 42 rows are **web-sourced** (Batch 1–4 = 32 + Batch 7 = 1 + Batch 8 = 3 + **Batch 9 = 6**): every one carries a real, verified web URL (nuforc.org, ufo-hunters.com, theblackvault.com, ufologie.patrickgross.org/URECAT, **inexplicata.blogspot.com**).
...
- **No prior-batch row was dropped.** All 11 Batch-1, 9 Batch-2, 8 Batch-3, 4 Batch-4, 42 Batch-6, 1 Batch-7, 3 Batch-8 rows are present and unchanged; **Batch 9 grew from 5 (v1) to 6 (v2)** — the +1 row is PRUAP-GAP-INX-2006-001 (Sep 18-19 2006 Northern PR cluster from the 2005-mid-2007 tail crawl, this dispatch). Total 84. **Schema consistent: programmatic re-verification 2026-05-31 confirmed 0 rows with a wrong column count and 84/84 unique `provisional_case_id` values.**
...
- **`archived_url` final populated-vs-blank count:** of the **84 candidate rows, 80 carry a real Wayback `archived_url` (or Batch-6 intentional `n/a-PDF`); 4 are blank.**
  - **archived_url with real `http://web.archive.org/web/...` snapshot: 38** (33 from prior dispatches + **5 added this dispatch via Chrome+availability API**: PRUAP-GAP-INX-2023-001, 2023-002, 2007-001, 2009-001, **2006-001 [new this dispatch]**).
  - archived_url = "(n/a — local PDF/book source)" Batch 6: 42 (unchanged).
  - **archived_url blank: 4** — 3 from Batches 1–4 (PRUAP-GAP-1976-001, PRUAP-GAP-2017-001, PRUAP-GAP-CL-2018-001) + **PRUAP-GAP-INX-1973-001 (the 2026-05-12 Impala URL — Save Page Now hung twice via Chrome this dispatch; same hung-Wayback pattern as the §11 4-of-27 BLANK cohort)**. Per the no-fabrication rule, this row's `archived_url` remains the explicit blocked-this-session note rather than a fabricated URL.
...
## 1. Executive Coverage Summary (combined, Batches 1–9 — 2026-05-31 v2)

Total candidate rows across all batches: **84** — Batch 1 = 11, Batch 2 = 9, Batch 3 = 8, Batch 4 = 4, **Batch 6 = 42**, **Batch 7 = 1**, **Batch 8 = 3**, **Batch 9 = 6** (5 from the 2026-05-28 v1 + 1 from the 2026-05-31 tail-crawl follow-up).
...
End of consolidated dispatch result (Batches 1–9, 2026-05-31 final).
